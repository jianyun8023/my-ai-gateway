...
以下在 **2026-09-07、main `c4da717`** 核对；用于避免重复实现，后续任务仍需检查自己的分支。
...
- 开放任务是 [#113 测试总计划](https://github.com/jianyun8023/my-ai-gateway/issues/113)、[#120 性能基线与故障注入](https://github.com/jianyun8023/my-ai-gateway/issues/120)、[#110 事件中心设计讨论](https://github.com/jianyun8023/my-ai-gateway/issues/110)。`test-faults` / `test-load` 仍是退出码 1 的占位任务，包含它们的 `test-all-offline` 不能视为已完成的全绿门禁。
- #1、#50 已关闭；TODO / 设计文档仍有 OTel 待办等过时描述。#96 / #97 / #98 已关闭，但验收清单的生产复验未勾选，#98 评论仍明确缺根因结论；不得据关闭状态宣称生产问题已经验证解决。
- 已知日志缺口：`src/proxy/usage.rs::log_sse_extraction_failure` 在 warn 级输出响应首尾各最多 512 字节的 Base64，与默认不记录正文片段的目标有差距；本次扫描仅记录，未修复，不能将现有编码预览视为脱敏范例。
