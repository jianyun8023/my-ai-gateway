...
两个 job 的 `mise-action@v4` 显式固定 mise 安装器版本为 `2026.9.2`。PR #178 的首次运行与重跑曾因自动选择的 `2026.9.3` Release/下载资产不存在而在工具链安装阶段返回 404，尚未执行源码检查；固定已发布版本用于恢复必要门禁，不改变 Rust/Node 版本、检查内容或工作流权限。后续更新安装器时应同时更新这两个步骤并确认对应平台的官方 Release 资产已发布。
