...
- 请求事件默认显示时间、模型、提供商、客户端、状态、延迟、Token、TPS、缓存九列（另有操作列）。模型只合并表格展示，筛选、详情和导出保留逻辑/上游两个字段；名称差异仅表示映射，回退图标依据事件的 `fallback` 及原因。重试为可选列，零重试且无回退显示 `—`。列偏好使用 v5，新版本首次恢复这些默认列，不迁移旧列标识。
- 时间以本地 `HH:mm:ss` 配合可见日期显示，完整时间保留在提示和详情。延迟条相对已加载请求的最大有效总延迟缩放；3 秒/10 秒颜色分段仅供扫读，不代表 SLA，缺失与真实零值分开处理。新加载事件仅短暂高亮单元格背景，1.5 秒后清除；虚拟行的位移仍由 TanStack 管理，并遵守减少动态效果偏好。
