...
| 项目 | 原型 | 迁移前 | 本批结果及保留差异 |
| --- | --- | --- | --- |
| 总览趋势 | 140px Input/Output 堆叠柱；绿色深浅区分 | 310px 双轴折线，默认 Total Token | 默认增加真实 Input/Output 堆叠柱；240px 保留时间轴、数值轴及图例空间。其他六项指标仍可切换折线；双轴标注各自单位。品牌 accent 与 muted 区分两条序列，未逐像素复制双色绿色 |
...
| Token 构成 | 四类 Token 拼成 100% 堆叠 | 独立进度条，零类被隐藏，未说明重叠 | 继续独立展示 Input、Output、Reasoning、Cache Read、Cache Creation；保留零值并解释重叠与 missing。上游 Total 不按各类重新求和；不能复制原型的相加语义 |
...
本批的 DOM/适配器回归覆盖堆叠与独立总量、切换指标/双轴、展开精确表、分布排序/截取/分母、零值、Token 重叠与缺失提示，以及来源延迟/P95 缺失与真实零值。浏览器视觉验收结果另记于[迁移清单](mantine-migration.md)第五批；不要以 DOM 测试替代 canvas、暗色和窄屏视觉证据。
