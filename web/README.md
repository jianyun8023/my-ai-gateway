...
“模型与路由”每个逻辑模型仅显示一个条目，直接展示主线路、备用线路、协议与运行状态。编辑抽屉按线路顺序保存配置，并提供模型启用状态、总超时和重试次数；协议支持只读，高级实现信息默认折叠。组件复用与范围见 [V3 实施说明](../docs/design/model-routing-ui/implementation-v3.md)。
