...
export function TokenComposition({ summary }: { summary: UsageSummaryViewModel }) {
  ...
  const formatTokens = (value: number) => unreported ? { display: '—', exact: t('usage.composition.unreported') } : formatCompactWithTitle(value);
  ...
  ].map(part => ({ ...part, label: t(`usage.legend.${part.key}`), formatted: formatTokens(part.value), share: combined > 0 ? part.value / combined * 100 : 0 }));
  const details = [
  ...
  const chartLabel = combined > 0
    ? parts.map(part => `${part.label} ${part.formatted.exact} Token (${formatPercent(part.share / 100)})`).join(' · ')
    ...
    <div className={styles.tokenComposition}>
      ...
      <div className={styles.compositionChart}>
        ...
        <dl className={styles.compositionLegend}>{parts.map(part => <div key={part.key}>
          ...
          <dd><strong title={part.formatted.exact}>{part.formatted.display}</strong><span>{combined > 0 ? formatPercent(part.share / 100) : '—'}</span></dd>
        </div>)}</dl>
      ...
      <dl className={styles.compositionDetails}>{details.map(detail => {
        const formatted = formatTokens(detail.value);
