...
describe('console navigation', () => {
  it('resolves the seven production navigation hashes', () => {
    expect(CONSOLE_PAGES).toEqual(['overview', 'analysis', 'events', 'runtime-events', 'sources', 'models', 'settings']);
  ...
  it('separates usage and management pages using the same route model', () => {
    ...
    expect(CONSOLE_PAGES.filter(page => !isUsagePage(page))).toEqual(['runtime-events', 'sources', 'models', 'settings']);
  });
