...
- 图标按钮必须有名称；加载按钮禁用并暴露 `aria-busy`；可交互元素提供 `focus-visible`。需要在禁用状态显示 Tooltip 的图标按钮使用可聚焦的 `aria-disabled` / `data-disabled`，并在公共入口阻止自身与父行激活。
...
本批使用 Mantine **9.6.0**（core/hooks/notifications 精确锁定，React 19.2 兼容）。基础组件负责交互，公共组合负责项目契约，业务组件负责数据与提交。完整八页清单、批次和证据见 [迁移记录](docs/mantine-migration.md)。主题、浮层、公共控件、页面筛选、反馈/状态、卡片、图表、控制面表格和虚拟事件列表均已接入；第十三批已完成剩余组件扫描、八页核心入口与适用浮层组合的最终验收。生产写入、真实 Provider、屏幕阅读器人工朗读与帧率基准仍按独立环境边界披露，不作为本次前端收尾新增条件。
...
- closeDisabled 同时保护关闭按钮、Esc 和遮罩点击，提交按钮仍由业务 busy 防重复触发。没有提交中的普通浮层允许 Esc 和外部点击关闭。Select 展开时第一次 Esc 只关闭其 Combobox，第二次才交给父 Modal/Drawer。
...
- 列偏好 Popover 使用 Portal、视口自动定位和 focus trap；交互内容使用 Popover，纯文本提示使用 Tooltip。公共字段选择器和趋势指标均采用 Mantine Select，Portal 层级、flip/shift、禁用项、内部滚动及减少动画由统一主题配置；Modal 与 Drawer 内的 Select 已验证 Esc 层级和焦点。当前没有 Menu、Popover 内 Select、Modal 内 Popover 或 Drawer 内二次确认等生产组合，这些记为 N/A，不构造虚假验收场景。
...
- Virtual Key 创建、读取、轮换以结果 Modal 为唯一结果入口，复制状态只在 Modal 内播报，不把 Key 正文放入 live region 或通知。创建/轮换成功先关闭表单 Modal，在其退出回调后再打开结果 Modal，避免同一渲染周期争抢焦点；结果关闭后返回稳定的创建/轮换触发器。轮换结果保留旧 Key 最晚有效时间（取重叠期和原到期时间的较早者）或立即失效说明；复制不清除时限。关闭即从 DOM 清除 Key 与复制状态，不等待退出动画。
