...
| `Input` / `Select` | 带右侧元素输入与自定义选择器等专项需求 |
...
| `Modal` | dialog / drawer、焦点约束与恢复、滚动锁 |

通用组件在 `web/src/components/ui`，不反向依赖 API 或控制面模块。公共样式由 [ConsolePrimitives.module.scss](web/src/components/ui/ConsolePrimitives.module.scss) 与 [components.scss](web/src/styles/components.scss) 管理。`features/control-plane/shared.tsx` 保留协议标签、错误映射、确认流程和业务布局。
